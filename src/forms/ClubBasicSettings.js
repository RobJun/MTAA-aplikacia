...
const BasicSettings = ({formImage,selectImage,resetImage, onChange,error,form,title,onPress,defaultImage, scrollable=false, visible = false, editable = true}) =>{
    ...
    const child =(
            ...
            <CredentialInput label={'Name'} placeholder = {"Enter club name, max 20 characters"} multi = {false} value={form.name} onChangeText={(value)=>{onChange({name:'name',value})}} error={error.name} editable={editable}/>
